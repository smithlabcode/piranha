...
class Test:
  ...
  def __str__(self):
    return "name=" + self.name + " command=" + self.command + " expect=" +\
           self.expected
...
def compareStrings(s1, s2, absTol=None, relTol=None, logfile=None):
  ...
  if len(numbers_s1) != len(numbers_s2):
    ...
    logfile.write("expected numbers: " +
                  ", ".join([str(x) for x in numbers_s2]) + "\n")
    logfile.write("got numbers: " +
                  ", ".join([str(x) for x in numbers_s1]) + "\n")
    return False
  ...
  if len(strings_s1) != len(strings_s2):
    ...
    logfile.write("expected strings: " +
                  ", ".join([str(x) for x in numbers_s2]) + "\n")
...
def loadTests(fn):
  ...
  res = []
  dt_nm = "regressionTests"
  tests = dom.getElementsByTagName(dt_nm)[0].getElementsByTagName('test')
  for test in tests:
