/****   
  ...
  --------------------
  This is abstract base class defining an interface for GLMs; it inherits 
  a lot from RegressionModel and is basically just a place to put the IRLS
  algorithm and its accoutrement so that it needn't be re-written for each 
  GLM implementation.  
  --------------------
  ...
  TODO:          It probably makes sense to move the coefficients vector
                 up a level in the heirarchy, to RegressionModel (..?), after
